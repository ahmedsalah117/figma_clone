  ...
  useEffect(() => {
    ...
    const newComment = (e: MouseEvent) => {
      ...
      if (creatingCommentState === "placed") {
        ...
        // const isClickOnComposer =
        //   (e as any)._savedComposedPath ===
        //   e.composedPath().some((el: any) => {
        //     return el.classList?.contains("lb-composer-editor-actions");
        //   });

        // So I replaced it with the following code...

        const isClickOnComposer = e.composedPath().some((el: any) => {
          return el.classList?.contains("lb-composer-footer");
        });
